...
import static leojay.tools.java.QLog.w;
...
public final class ReadProperties {
    ...
    public Properties initConfig() throws MyToolsException {
        ...
        if (properties == null) {
            if (urlMode == InputMode.ABS) {
                QLog.i(ReadProperties.class, "没有该文件，将创建默认文件!");
     ...
     * @param key        键
     * @return 获取 properties 里 key 的值，若不存在，创建并默认为："default"
     */
    ...
    private Properties readConfig(InputMode mode, String url) {
        ...
        if (in == null) {
            ...
            QLog.i(this, "已找到该路径文件: " + url);
            try {
                QLog.i(this, "正在读取数据！");
                properties.load(in);
                in.close();
                QLog.i(this, "读取文件成功，并已关闭输入流");
            } catch (IOException e) {
    ...
    private void writeConfig(Properties properties, String configName, String url) {
        ...
        if (!f.exists()) {
            ...
            mkdirs = f.mkdirs();
            if (mkdirs) {
                QLog.i(this, "配置文件目录创建成功！");
            } else {
                QLog.i(this, "配置文件夹创建失败！");
            }
        } else {
            QLog.i(this, "配置文件目录存在。");
        }
        QLog.i(this, "正在写入配置文件！");
//        if (properties != null) {
